...
/***/ function(module, exports, __webpack_require__) {
	...
	var controller = function ($, window, document) {
	    ...
	    IndexForSpanList,
	        //默认圈数
	    addPrefix = _userAgent2.default.prefix,
	        //默认为chorme因为QQ浏览器和微信浏览器都是chorme内核
	    touchClient = _userAgent2.default.appEnvironment === "pc" ? false : true,
	        //判断是手机端还是客户端;
	    ...
	    openTime,
	        //开始创建时间
	    RecordPrvTimeForAjax,
	        //= now;//避免重复请求必须记录时间与上一个时间不同
	    //页面初始化;
	    $spanBox = $(".timeBox"),
	        deg = 0;
	    var data;
	    ...
	    var TIMESHARFT = { //用来处理时间的逻辑
	        ...
	        touchStartFn: function touchStartFn(addPrefix) {
	        ...
	        touchMoveFn: function touchMoveFn(client, addPrefix) {
	            if (client > startX) {
	        ...
	        touchEnd: function touchEnd(addPrefix) {
	        ...
	        //但触摸屏幕时调用左右移动方法
	        elasticDampingRing: function elasticDampingRing(addPrefix) {
	        ...
	        //具体操作当手离开圆盘时候的弹性回落方法
	        elasticDampingRingLogic: function elasticDampingRingLogic(directionMove) {
	            var _degMer,
	                _MathabsDegMer,
	                //角度的绝对值
	            ...
	            var _MoveFn = directionMove === "next",
	                _judge = _MathabsDegMer >= 30,
	                //大于30度就认为可以向前跳一个各了
	            _difference = _deg - _degMer,
	                //每次都是60的整数倍
	            _nextCallback;
	            ...
	            if (_degMer > 0) {
	                ...
	                _nextCallback = _MoveFn && _judge ? 'next' : _MoveFn && !_judge ? null : !_MoveFn && _judge ? null : 'prv';
	            } else if (_degMer < 0) {
	                deg = _judge ? _difference - 60 : _difference;
	        ...
	        setAttributeForCircle: function setAttributeForCircle(deg) {
	        ...
	        //与页面操作要绑定的部分
	        prvOrNextChangeHtml: function prvOrNextChangeHtml(datetimeChild, _prvOrNext) {
	            if (!_prvOrNext) {
	                return;
	            };
	        ...
	        controlIntervalMove: function controlIntervalMove(millisec, searchValueFn) {
	            var self = this;
	            if (this.turnsOfInterval) {
	        ...
	        prvOrNextMove: function prvOrNextMove(deg, prvOrNext) {
	        ...
	        searchOldValueFn: function searchOldValueFn() {
	            deg -= this.x;
	            this['prvOrNextMove'](deg, 'prv');
	        },
	
	        searchNewValueFn: function searchNewValueFn() {
	            deg += this.x;
	            this['prvOrNextMove'](deg, 'next');
	        },
	
	        fnTimeout: function fnTimeout() {
	            var self = this;
	            setTimeout(function () {
...
/***/ }
/******/ ]);
//# sourceMappingURL=pugin1.js.map